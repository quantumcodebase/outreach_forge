...
export function ErrorState({ title, description, retry }: { title: string; description: string; retry?: () => void }) {
  return (
    <div className="rounded-lg border border-amber-200/20 bg-amber-500/5 p-3">
      <div className="flex items-start gap-2">
        <span className="mt-0.5 text-xs text-amber-300">⚠</span>
        <div className="min-w-0">
          <p className="text-sm font-medium text-amber-100">{title}</p>
          <p className="mt-0.5 text-xs text-amber-200/80">{description}</p>
          {retry ? (
            <button onClick={retry} className="mt-2 rounded-md border border-amber-100/20 px-2.5 py-1 text-xs text-amber-50 hover:bg-amber-500/10">
              Retry
            </button>
          ) : null}
        </div>
  ...
  return (
    <div className="space-y-2">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="h-12 animate-pulse rounded-lg border border-white/10 bg-white/[0.03]" />
      ))}
    </div>
  );
}
