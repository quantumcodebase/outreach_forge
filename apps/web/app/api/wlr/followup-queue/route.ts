...
export async function GET(req: Request) {
  ...
  const promotionIds = [...new Set(rows.map((r) => r.promotion_id).filter(Boolean))] as string[];

  const [leads, recipes, qualifications, promotions] = await Promise.all([
    leadIds.length ? prisma.leads.findMany({ where: { id: { in: leadIds } }, select: { id: true, email: true, company: true, title: true } }) : [],
    ...
    promotionIds.length ? prisma.wlr_promotion_queue.findMany({ where: { id: { in: promotionIds } } }) : [],
  ]);
  ...
  return NextResponse.json({
    items: rows.map((r) => ({
      ...
      promotion: r.promotion_id ? promotionMap.get(r.promotion_id) || null : null,
    }))
  });
}
...
export async function POST(req: Request) {
  ...
  const followupStatus = String(body?.followup_status || 'ready').trim();
  ...
  const [lead, recipe, qualification, promotion] = await Promise.all([
    ...
    prisma.wlr_promotion_queue.findUnique({ where: { id: promotionId }, select: { id: true, lead_id: true } }),
  ]);

  if (!lead) return NextResponse.json({ error: 'lead_not_found' }, { status: 404 });
  if (recipeId && !recipe) return NextResponse.json({ error: 'recipe_not_found' }, { status: 404 });
  if (qualificationId && !qualification) return NextResponse.json({ error: 'qualification_not_found' }, { status: 404 });
  if (!promotion) return NextResponse.json({ error: 'promotion_not_found' }, { status: 404 });
  if (promotion.lead_id !== leadId) return NextResponse.json({ error: 'promotion_lead_mismatch' }, { status: 400 });

  const item = await prisma.wlr_followup_queue.upsert({
